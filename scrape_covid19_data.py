...
class DataScraper:
    ...
    def parse(self, in_memory = False):
        ...
        self.publication_date = caption_info_table[caption_info_table.find(token)+len(token):]
        ...
        if not in_memory:
            self.write('canada_covid19.csv', 'canada_covid19_meta.csv', values_parsed, columns, self.publication_date)

        self.data_frame = pd.DataFrame(values_parsed, columns = columns)
        
    def get_parsed_data(self):
        return self.data_frame, self.publication_date
...
if __name__ == "__main__":
    ...
    data, current = scraper.get_parsed_data()
